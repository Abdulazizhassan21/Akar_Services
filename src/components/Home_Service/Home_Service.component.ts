...
@Component({
  selector: 'app-Home_Service',
  templateUrl: './Home_Service.component.html',
  styleUrls: ['./Home_Service.component.css'],
  standalone: true,
  imports: [FormsModule, CommonModule]
})
export class Home_ServiceComponent implements OnInit {
  ...
  fetchWorkers(categoryId: number) {
    ...
    this._WorkersService.FeachDataWorker(categoryId).subscribe({
      ...
      error: (err) => {
        this.isLoading.set(false);
        console.error("خطأ أثناء جلب عمال خدمات المنزل:", err);
